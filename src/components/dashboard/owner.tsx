...
import NoStoreSvg from "../../../public/no-shop.svg";
    ...
    <>
      <div className="border-b border-dashed border-border-base mb-5 sm:mb-8 pb-8">
        <h1 className="text-lg font-semibold text-heading">
          {t("common:sidebar-nav-item-my-stores")}
        </h1>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 3xl:grid-cols-5 gap-5">
        {data?.stores?.map((myStore: any, idx: number) => (
          <StoreCard store={myStore} key={idx} />
        ))}
      </div>

      {!data?.managed_store && !data?.stores?.length ? (
